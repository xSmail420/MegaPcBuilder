...
\n\n
{ 'PROCESSEUR' : { 'lien': string }, 'REFROIDISSEMENT': { 'lien': string }, 'CARTE MÈRE': { 'lien': string }, 'BARETTE MÉMOIRE': { 'lien': string }, 'ALIMENTATION': { 'lien': string }, 'STORAGE': { 'lien': string },'VENTILATEUR': { 'lien': string }, 'CARTE GRAPHIQUE': { 'lien': string } }
\n\n
        START CONTEXT BLOCK
        ${JSON.stringify(context)}
        END OF CONTEXT BLOCK
        EVERY "lien: " attribute in the response is provided in the CONTEXT BLOCK.

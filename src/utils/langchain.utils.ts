...
export class LangChainUtils {
   ...
   */
  public static createChatMessageHistory(
   ...
   * @returns A summarize
   */
  public static async summarizeMessages(
    messages: MessageModel[]
  ): Promise<String> {
    ...
    const summary = await summary_memory.predictNewSummary(
      await summary_memory.chatHistory.getMessages(),
      ""
    );
    return summary;
  }
  ...
  public static async getResponse(
    ...
    try {
      ...
      const personalisationSnapshot = await db
        .collection("userPersonalisation")
        .doc(userSnapshot.data()?.personalisation_id)
        .get();
      const personalisationData =
        personalisationSnapshot.data() as UserPersonalisation;
      ...
      const userPersonalisationString =
        LangChainUtils.formatUserPersonalisation(personalisationData);
  ...
  static formatUserPersonalisation(
    userPersonalisation: UserPersonalisation
  ): string {
    const formattedAnswers = userPersonalisation.answers.map(
      ({ question, answer }) => {
